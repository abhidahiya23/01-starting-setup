...
fig, ax = plt.subplots(1,2,figsize=(11.69,8.27),gridspec_kw={'width_ratios': [3, 1]})
#fig.set_figheight(8)
#fig.set_figwidth(8)
bottom = np.zeros(2)
...
for boolean, weights in weights.items():
    p = ax[1].bar(areas, weights, width, label=boolean, bottom=bottom)
    bottom += weights
...
#print(min_value)
#print(max_value)
ax[1].set_yticks([500, 1000, 1500, 2000, 2500, 3000,3500, 4000, 4500])
ax[1].yaxis.grid(True, linestyle='-', which='major',
                   color='grey', alpha=.2)
ax[1].set_title("Segment Distribution at Area Level")
ax[1].set_ylabel('Segment Count')
...
bottom1 = np.zeros(2)
for boolean1, weights1 in weights1.items():
    #print(boolean1)
    p1 = ax[0].bar(areas1, weights1, width1, label=boolean1, bottom=bottom1)
    bottom1 += weights1
    min_value = min(min_value,bottom[0])
    max_value = max(max_value,bottom[0])
...
#print(min_value)
#print(max_value)
ax[0].set_yticks([500, 1000, 1500, 2000, 2500, 3000,3500, 4000, 4500])
ax[0].yaxis.grid(True, linestyle='-', which='major',
                   color='grey', alpha=.2)
ax[0].set_title("Segment Distribution at Area Level")
ax[0].set_ylabel('Segment Count')
